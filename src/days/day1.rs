...
// Used for both parts, so put into dedicated function
// Could probably be optimized for compile-time parsing
fn get_input_vectors() -> (Vec<u32>, Vec<u32>) {
    let input = include_str!("../../input/day1.txt");

    // Parse the input string into a vector of u32s
    // For string parsing simplicity, this collapses the 2D array above into a 1D array
    let values: Vec<u32> = input
        .split_ascii_whitespace() // splits at spaces and newlines
        .map(|x| x.parse().unwrap())
        .collect();

    // Get the 'left column' of the collapsed 1D array (even elements)
    let left: Vec<u32> = values
        .iter()
        .step_by(2)
        .copied()
        .collect();

    // Get the 'right column' of the collapsed 1D array (odd elements)
    let right: Vec<u32> = values
        .iter()
        .skip(1) // get odd values
        .step_by(2)
        .copied()
        .collect();

    assert_eq!(left.len(), right.len(), "left and right vecs have mismatched size");
    assert_eq!(left.len() + right.len(), values.len(), "column vectors incorrectly parsed from flat array");
...
impl AdventSolution for Day1 {
    ...
    //
    // Upon pouring into the office, everyone confirms that the Chief Historian is indeed nowhere to
    // be found. Instead, the Elves discover an assortment of notes and lists of historically significant
    // locations! This seems to be the planning the Chief Historian was doing before he left.
    ...
    //
    // The second-smallest number in the left list is 2, and the second-smallest number in the right list
    // is another 3. The distance between them is 1.
    //
    ...
    // To find the total distance between the left list and the right list, add up the distances between all
    // of the pairs you found. In the example above, this is 2 + 1 + 0 + 1 + 2 + 5, a total distance of 11!
    //
    // Your actual left and right lists contain many location IDs.
    // What is the total distance between your lists?
    fn part1(&self) -> u32 {
        let (mut left, mut right) = get_input_vectors();

        // Sort vectors in ascending order for pairwise comparison
        left.sort();
        right.sort();

        // Find the pairwise distance between column pairs and sum into a single distance value
    ...
    fn part2(&self) -> u32 {
        let (left, right) = get_input_vectors();

        // Create map where the key is a unique element in left and the value is the similarity score
        let mut map: HashMap<u32, u32> = HashMap::from_iter(left.into_iter().map(|x| (x, 0)));

        // Update similarity score if right element exists as key
        right.iter().for_each(|x| {
            if let Some(score) = map.get_mut(x) {
                *score += x;
            }
        });

        // Sum all similarity values into one
        let similarity = map.into_values().sum();
